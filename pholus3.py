                        ...
                        if block.haslayer(DNSRROPT):
                            ...
                            while isinstance(block,DNSRR):#Somewhat equivalent: while not isinstance(an, NoPayload):
                                ...
                                if dnsrr.rclass==32769:
                                    res = res0 + " DNS Resource Record: "+ dnsrr.rrname.decode("utf-8") + " " + dns_type[dnsrr.type] +" QU Class:IN "+dnsrr.rdata
                                elif dnsrr.rclass==1:
                                    res = res0 + " DNS Resource Record: " + dnsrr.rrname.decode("utf-8") + " "+ dns_type[dnsrr.type] + " QM Class:IN "+dnsrr.rdata
                                ...
                                if dnsrr.type==33:#SRV Record
                                    ...
                                    res = res0 + " Additional_Record: "+ dnsrr.rrname.decode("utf-8") + " " + dns_type[dnsrr.type]+" " + str(dnsrr.rclass) + " priority="+str(int(priority,16))+" weight="+str(int(weight,16))+" port="+str(int(port_number,16))+" target="+str(dnsrr.rdata)[6::]
                                else:
                                    rdata=dnsrr.rdata
                                    ...
                                    if "._tcp." not in rdata and "._udp." not in rdata:
                                        if rdata == "_dhnap.":
                                            rdata=rdata+"_tcp."
                                    res = res0 + " Additional_Record: "+ dnsrr.rrname.decode("utf-8") + " " + dns_type[dnsrr.type]+" " + str(dnsrr.rclass) + ' "' +rdata+'"'
                                if show_ttl:
